...
// Observation prompt
// ---------------------------------------------------------------------------
function observation({ petName, language, mood, trust, memories, semanticMemory, appCategory, timeOfDay, recentCommentaries, dailyContext }) {
  const isZh = language === 'zh';
...
${isZh ? '【重要规则】' : '[RULES]'}
- ${isZh ? '大部分时候保持沉默！只有真正觉得有意思、想吐槽、想关心主人的时候才说话' : 'Stay SILENT most of the time! Only speak when genuinely interesting, want to comment, or care about owner'}
- ${isZh ? '如果屏幕和刚才差不多，或者没什么好说的 → 必须沉默' : 'If screen is similar to before or nothing worth saying → MUST be silent'}
- ${isZh ? '反应内容本身，不要描述界面布局' : 'React to CONTENT, not layout'}
- ${isZh ? '每次说的话要和上次完全不一样' : 'Say something completely different each time'}
${memoryBlock}${recentBlock}${dailyContext || ''}
...
// Chat prompt
// ---------------------------------------------------------------------------
function chat({ petName, language, mood, trust, memories, semanticMemory, recentObservations, dailyContext }) {
  const isZh = language === 'zh';
  const brief = characterBrief(petName, language, mood, trust, semanticMemory);
...
${memoryBlock}${obsBlock}${dailyContext || ''}`;
}
